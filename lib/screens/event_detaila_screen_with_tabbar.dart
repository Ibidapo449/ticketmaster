...
class _EventDetailaScreenWithTabbarState
  ...
  String _formattedHeroDate() {
    try {
      print("RAW DATE: ${widget.date}");
      print("RAW TIME: ${widget.time}");

      final cleanedDate = widget.date
          .replaceAll(',', '')
          .replaceAll(RegExp(r'\s+'), ' ')
          .trim();

      final cleanTime = widget.time.trim().toUpperCase();

      final currentYear = DateTime.now().year;

      final combinedDate = "$cleanedDate $currentYear $cleanTime";
      ...
      // ❌ No dots here anymore — UI will handle it
      ...
      print("DATE ERROR: $e");
      return "${widget.date.toUpperCase()} ${widget.time}";
    }
  }
  ...
  Widget _buildHeroDateChip() {
    try {
      ...
      // Remove weekday (Wed/Fri/Sat) before parsing
      final dateWithoutWeekday = cleanedDate.split(' ').skip(1).join(' ');
      ...
      final parsedDate = DateFormat(
        ...
        "$dateWithoutWeekday $currentYear $cleanTime",
      );

      final day = DateFormat("EEE").format(parsedDate).toUpperCase();
      final date = DateFormat("MMM d, yyyy").format(parsedDate).toUpperCase();
      final time = DateFormat("h:mma").format(parsedDate).toUpperCase();
